...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (mode == 1) status++;
        if (status == 0) {
            cm.sendSimple("\r\n\r\n\r\n#L0##r#e<增加强化次数>#n#d500000点卷 #r增加装备砸卷次数#l");
        } else if (status == 1) {
            if (selection == 0) { 
                fstype = 1;
                cm.sendNext("\r\n\r\n\r\n你目前选择的是 #d500000点卷 #r增加装备砸卷次数#k\r\n\r\n#r#e注意：20%机率成功");
            } else if (selection == 1) {
                fstype = 2;
            ...
            if (fstype == 13) {
             ...
             cm.全服黄色喇叭("[装备提升] : 恭喜玩家 "+cm.getPlayer().getName()+" 在装备强化处,使用100万点卷为武器提升了1次砸卷次数！")
                    cm.dispose();
                } else {
                    cm.sendOk("真遗憾，升级失败");
				        //cm.gainItem(4001168,-1);
