...
var 感叹号 = "#fUI/UIWindow/Quest/icon0#";
	...
	if (status == 0) {
	    ...
	    var textz = "\r\n";
		textz += "#r#L0#" + 草莓 + "  #e新人礼包#e #b(#d此礼包只能领取一次#b)#n#l\r\n\r\n";

		textz += "#r#L1#" + 草莓 + "  #e每日点券 #b(#d每天领取3W点券#b)#l#n\r\n\r\n";
		
		textz += "#r#L2#" + 草莓 + "  #e70级礼包 #b(#d此礼包只能领取一次#b)#l#n\r\n\r\n";
		
		textz += "#r#L3#" + 草莓 + "  #e庆祝RNG喜提16强 #l#n\r\n\r\n";
	...
	if (selection == 0) {
		...
		if(cm.getBossLog("每日领取点券") >= 1){
			cm.sendOk("每天只能领取一次")
			cm.dispose();
			return
		}
		cm.gainNX(30000);
		cm.sendOk("#r成功领取每日点券，每天可以领取 ！");
	    cm.setBossLog("每日领取点券");
		cm.全服黄色喇叭("[每日点券] : 玩家 "+cm.getPlayer().getName()+" 在[新人礼包]处免费领取了3W点券")
		cm.dispose();
		return

}else if (selection == 2) {
		cm.dispose();
        cm.openNpc(9900004, 5004);

}else if (selection == 3){
		if(cm.getPlayer().getOneTimeLog("RNG庆祝") >= 1){
			cm.sendOk("只能领取一次")
			cm.dispose();
			return
		}
		cm.给属性装备(1022048, 0, 0, 30, 30, 30, 30, 2215, 2215, 30, 30,0, 0, 0, 0, 0, 0, 0);
		cm.全服黄色喇叭("[庆祝RNG喜提16强] : 玩家 "+cm.getPlayer().getName()+" 领取了群主发放的庆祝RNG喜提16强奖励，获得[HP+小虎][六维+30]的透明眼镜，RNG牛逼！")
		cm.sendOk("领取成功")
		cm.getPlayer().setOneTimeLog("RNG庆祝");
		cm.dispose();
		return
        cm.openNpc(9900004, 5005);
