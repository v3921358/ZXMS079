...
function action(k, e, s) {
    ...
    var 金币 = cm.判断金币();
    var 金币拍卖行开关 = cm.GetPiot("金币拍卖行开关", "1");
    ...
    status++;
    if (status == 0) {
        character_auctionItems = cm.auction_findByCharacterId1();
        auctionPoint = cm.auction_getAuctionPoint1();
        var c = 0;
        var b = 0;
        var h = 0;
        if (character_auctionItems != null && !character_auctionItems.isEmpty()) {
            for (var n = 0; n < character_auctionItems.size(); n++) {
                ...
                if (p.getAuctionState1().getState() == 0) {
                    c++;
                } else {
                    if (p.getAuctionState1().getState() == 1) {
                        b++;
                    } else {
                        if (p.getAuctionState1().getState() == 2) {
                            h++;
                        }
                    }
                }
            ...
            if (status == 2) {
                ...
                if (sel_0 == 21) {
                    /*
                     金币兑换交易点
                     */
                    cm.sendGetNumber("\r\n#n" + 蘑菇 + " 市场公告:#r" + Message[Math.floor(Math.random() * Message.length)] + "#k\r\n\r\n#b提示：你当前可以兑换 #r" + 金币 + "#k #b交易点\r\n提示：你当前可以兑换 #r" + (auctionPoint == null ? 0 : auctionPoint.getPoint()) + "#k #b金币#d\r\n\r\n#k请输入你要兑换的交易点数量:#k", 1, 1, 1000000000);
                } else {
                    if (sel_0 == 22) {
                        var g = cm.auction_findByItemType1(sel_1);
                        if (g == null || g.size() < 1) {
                            ...
                            if (sel_1 == 1) {
                                ...
                                if (sel_1 == 2) {
                                    ...
                                    for (var n = 0; n < character_auctionItems.size(); n++) {
                                        ...
                                        j += "#r" + (r.getAuctionState1().getState() == 2 ? "\r\n" : "#L" + r.getId() + "#") + "#v" + r.getItem().getItemId() + "##b 数量:#r" + r.getQuantity() + "#b价格:#r" + r.getPrice() + " #b状态:" + (r.getAuctionState1().getState() == 0 ? "#k" : r.getAuctionState1().getState() == 1 ? "#r" : "#d") + r.getAuctionState1() + (r.getAuctionState1().getState() == 2 ? "[" + r.getBuyerName() + "]" : "#l") + "\r\n";
                                    }
                                    cm.sendSimple(j);
                                }
                            }
                    ...
                    if (sel_0 == 21) {
                        if (sel_1 == 1) {
                            if (sel_2 * 金币兑换率 > cm.判断金币()) {
                                cm.sendOkS("金币不足:兑换" + sel_2 + "交易点需要" + sel_2 * 金币兑换率 + "金币", 3);
                                cm.对话结束();
                                return;
                            }
                            var o = cm.auction_addPoint1(sel_2);
                            if (o > -1) {
                                cm.gainMeso(-sel_2 * 金币兑换率);
                                cm.个人存档();
                            ...
                            if (sel_1 == 2) {
                                var l = cm.auction_getAuctionPoint1();
                        ...
                        if (sel_0 == 22) {
                            var r = cm.auction_findById1(sel_2);
                            if (r == null) {
                            ...
                            j += " \t状态 #b" + (r.getAuctionState1().getState() == 0 ? "#k" : r.getAuctionState1().getState() == 1 ? "#r" : "#d") + r.getAuctionState1() + (r.getAuctionState1().getState() == 2 ? "[" + r.getBuyerName() + "]" : "") + "#k\r\n\r\n";
                            ...
                            if (sel_0 == 23) {
                                if (sel_1 == 1) {
                                    ...
                                    if (sel_1 == 2) {
                                        if (sel_2 == 10001) {
                                            var d = cm.auction_deletePlayerSold1();
                                            if (d > -1) {
                                        ...
                                        var j = "";
                                        j += "#b道具#v" + r.getItem().getItemId() + "# 拍卖状态:" + (r.getAuctionState1().getState() == 0 ? "#k" : r.getAuctionState1().getState() == 1 ? "#r" : "#d") + r.getAuctionState1() + "\r\n\r\n";
                                        var m = parseInt(r.getItem().getItemId() / 1e6);
                                        if (m == 1) {
                                            j += showEquipState(r.getItem());
                                        }
                                        j += "\r\n\r\n";
                                        if (r.getAuctionState1().getState() == 0) {
                                            j += "#b#L2#" + 箭头 + "上架#l	 	#L3#" + 箭头 + "取回#l#k\r\n";


                                        } else {
                                            if (r.getAuctionState1().getState() == 1) {
                                                j += "#b#L1#" + 箭头 + "下架#l#k\r\n";
                                            } else {
                                                if (r.getAuctionState1().getState() == 2) {
                                                    j += "#b#L4#" + 箭头 + "移除#l#k\r\n";
                                                }
                                            }
                                        }
                                ...
                                if (sel_0 == 23) {
                                    if (sel_1 == 1) {
                                        ...
                                        if (sel_1 == 2) {
                                            ...
                                            if (sel_3 == 1) {
                                                if (r.getAuctionState1().getState() != 1) {
                                                    cm.sendOkS("不可操作的道具,道具状态为" + r.getAuctionState1() + " ,只有上架的才能下架", 3);
                                                    cm.对话结束();
                                                    return;
                                                }
                                                var q = cm.auction_soldOut1(sel_2);
                                                if (q == -5) {
                                                    cm.sendOkS("操作失败(" + q + ")不存在的道具", 3);
                                                    cm.对话结束();
                                                } else {
                                                ...
                                                if (sel_3 == 2) {
                                                    if (r.getAuctionState1().getState() != 0) {
                                                        cm.sendOkS("不可操作的道具,道具状态为" + r.getAuctionState1() + " ,只有下架的道具才能上架", 3);
                                                        cm.对话结束();
                                                        return;
                                                    }
                                                    ...
                                                    if (sel_3 == 3) {
                                                        if (r.getAuctionState1().getState() != 0) {
                                                            cm.sendOkS("不可操作的道具,道具状态为" + r.getAuctionState1() + " ,只有下架的道具才能取回", 3);
                                                            cm.对话结束();

                                                            return;
                                                        }
                                                        var q = cm.auction_takeOutAuctionItem1(sel_2);
                                                        if (q == -5) {
                                                            cm.sendOkS("操作失败(" + q + ")不存在的道具", 3);
                                                            cm.对话结束();
                                                        } else {
                                                        ...
                                                        if (sel_3 == 4) {
                                                            if (r.getAuctionState1().getState() != 2) {
                                                                cm.sendOkS("不可操作的道具,道具状态为" + r.getAuctionState1() + " ,只有已售的道具才能移除", 3);
                                                                cm.对话结束();
                                                                return;
                                                            }
                                                            var d = cm.auction_deletePlayerSold1();
                                ...
                                if (sel_0 == 22) {
                                    ...
                                    if (sel_0 == 23) {
                                        if (sel_1 == 1) {
                                            ...
                                            var a = cm.auction_putIn1(f, sel_4);
                                            if (a == -4) {
                                            ...
                                            if (sel_1 == 2) {
                                                ...
                                                if (sel_3 == 2) {
                                                    ...
                                                    var q = cm.auction_setPutaway1(sel_2, sel_4);
                                                    if (q == -5) {
                                                        cm.sendOkS("操作失败(" + q + ")不存在的道具", 3);
                                                        cm.对话结束();
                                                    } else {
                                                        if (q == -6) {
                                                            ...
                                                            if (q == -2) {
                                                                ...
                                                                if (q < 1) {
                                                                    ...
                                                                    //cm.worldMessage(2, "[拍卖行信息] : 玩家[ " + cm.getChar().getName() + " ]在金币拍卖行上架物品[ " + cm.getItemName(r.getItem().getItemId()) + " ] ");//[ #t" + cm.getItemName() + "# ]  
                                                                    cm.个人存档();
                                                                    cm.playerMessage(5, "[拍卖行]:操作成功，为您跳回拍卖界面。");
                                                                    cm.对话结束();
                                                                    cm.openNpc(9900004, 3);
