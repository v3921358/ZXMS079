...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        var selStr = "	  Hi~#b#h ##k我是星缘，这，是你想要的礼物吗？不过这个礼物，我只能在大年初一至初十才可以送给你哦。记得到时候找我领取。\r\n";
        ...
        switch (selection) {
            case 1:
				...
				cm.给物品(3010073,1);
				cm.全服黄色喇叭("[新年礼物] : 玩家 "+cm.getPlayer().getName()+" 在 "+cm.getPlayer().getMap().getMapName()+" 星缘处领取了新年礼物 baby品克缤。")
				cm.群输出信息("[新年礼物] : 玩家 "+cm.getPlayer().getName()+" 在 "+cm.getPlayer().getMap().getMapName()+" 星缘处领取了新年礼物 baby品克缤。");
                cm.对话结束();
                break;
        }
    }
