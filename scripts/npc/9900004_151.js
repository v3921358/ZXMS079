...
function action(mode, type, selection) {
    ...
    if (脚本执行 == 0) {
        ...
        cm.sendSimple("" + 说明文字 + "不过需要 "+cm.显示物品(音乐盒)+" x #b1#k 才可以进行点播。\r\n 	#r地图点播，在#m"+cm.判断地图()+"#的玩家可以听到#k。\r\n\r\n" + 文本信息 + "");
    } else if (脚本执行 == 1) {
        sels = selection;
        ...
        cm.changeMusic(BMG[sels]);
		cm.收物品(音乐盒,1);
        cm.说明文字("点播 #b" + BMG[sels] + "#k 成功。");
		if (cm.getPlayer().getGMLevel() > 0) {
			cm.地图公告("[音乐点播] : 神秘人点播了 " + BMG[sels] + " ");
		}else{
			cm.地图公告("[音乐点播] : 玩家 "+cm.getChar().getName()+" 点播了 " + BMG[sels] + " ");
		}
        cm.对话结束();
    } else {
