...
function action(mode, type, selection) {
	...
	if (status <= 0) {
        ...
        if (我的余额 >= 金额) {
            ...
            cm.说明文字("#r转账成功~#k\r\n\r\n\r\n转账账号 : #b" + 账号 + "#k\r\n金币金额 : #b" + 金额 + "#k");
			cm.私聊输出信息("您的银行账号 "+我的账号+" 于 "+cm.时间()+" 转出 "+金额+" 金币，目前该银行账号余额为 "+(我的余额 - parseInt(金额))+" 金币。（来自"+cm.开服名称()+"自由银行游戏转账）", "" + cm.账号ID取绑定QQ(cm.getPlayer().getClient().getAccID()) + "");
            cm.私聊输出信息("您的银行账号 "+账号+" 于 "+cm.时间()+" 转入 "+金额+" 金币，目前该银行账号余额为 "+(余额 + parseInt(金额))+" 金币。（来自"+cm.开服名称()+"自由银行游戏转账）", "" + cm.账号ID取绑定QQ(cm.角色ID取账号ID(cm.银行开户人(账号))) + "");                                              
        } else {
