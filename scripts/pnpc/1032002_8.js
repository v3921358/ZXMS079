...
function action(mode, type, selection) {
	...
	if (cm.Getcharacter7("" + 角色 + "", 101) > 0) {
		if(cm.getBossRank9("防爆卡",2)>0){
			cm.个人公告("你已启用附魔防暴，道具低于 70 级，生效概率为 30 %");
		}
	}
	if (cm.Getcharacter7("" + 角色 + "", 201) > 0) {
		if(cm.getBossRank9("必成卡",2)>0){
			cm.个人公告("你已启用附魔必成，道具低于 70 级，生效概率为 30 %");
		}
	}
    ...
    if (selection == 0) {
        var txt1 = '   Hi~#b#h ##k，请选择你要附魔的装备，给装备附魔的效果是随机的哦，不过你事先得打孔才行哦。（#r该附魔最大值高于普通附魔，但是附魔失败装备会被损坏#k）#n#k\r\n';
...
function 附魔(x) {
    ...
    if (cm.百分率(附魔成功率)) {
		...
		if(装备损坏==0){
			cm.脱掉并且销毁装备(x);
			cm.sendNext("" + cm.显示物品(ID) + "  附魔失败，道具被损坏!"+装备损坏);
		}else{
